...
_.extend(LivedataTest.ClientStream.prototype, {                                                                        // 31
  ...
  _launchConnection: function () {                                                                                     // 114
    ...
    var finalize = Meteor.bindEnvironment(function () {                                                                // 168
      if (stream.unpipe) {                                                                                             // 169
        stream.unpipe(client.io);                                                                                      // 170
      }                                                                                                                // 171
      stream.on('data', function () {                                                                                  // 172
        stream.destroy();                                                                                              // 173
      });                                                                                                              // 174
      stream.end();                                                                                                    // 175
      if (client === self.client) {                                                                                    // 176
        self._lostConnection();                                                                                        // 177
      }                                                                                                                // 178
    }, "finalizing stream");                                                                                           // 179
                                                                                                                       // 180
    stream.on('end', finalize);                                                                                        // 181
    stream.on('close', finalize);                                                                                      // 182
    client.on('close', finalize);                                                                                      // 183
                                                                                                                       // 184
    var onError = function (message) {                                                                                 // 185
      if (!self.options._dontPrintErrors)                                                                              // 186
        Meteor._debug("driver error", message);                                                                        // 187
                                                                                                                       // 188
      // Faye's 'error' object is not a JS error (and among other things,                                              // 189
      // doesn't stringify well). Convert it to one.                                                                   // 190
      self._lostConnection(new DDP.ConnectionError(message));                                                          // 191
    };                                                                                                                 // 192
                                                                                                                       // 193
    clientOnIfCurrent('error', 'driver error callback', function (error) {                                             // 194
      onError(error.message);                                                                                          // 195
    });                                                                                                                // 196
                                                                                                                       // 197
    stream.on('error', Meteor.bindEnvironment(function (error) {                                                       // 198
      if (client === self.client) {                                                                                    // 199
        onError('Network error: ' + wsUrl + ': ' + error.message);                                                     // 200
      }                                                                                                                // 201
      stream.end();                                                                                                    // 202
    }));                                                                                                               // 203
                                                                                                                       // 204
    clientOnIfCurrent('message', 'stream message callback', function (message) {                                       // 205
      // Ignore binary frames, where data is a Buffer                                                                  // 206
      if (typeof message.data !== "string")                                                                            // 207
        return;                                                                                                        // 208
      _.each(self.eventCallbacks.message, function (callback) {                                                        // 209
        callback(message.data);                                                                                        // 210
      });                                                                                                              // 211
    });                                                                                                                // 212
                                                                                                                       // 213
    stream.pipe(self.client.io);                                                                                       // 214
    self.client.io.pipe(stream);                                                                                       // 215
  },                                                                                                                   // 216
                                                                                                                       // 217
  _createSocket: function (wsUrl, onConnect) {                                                                         // 218
    var self = this;                                                                                                   // 219
    var urlModule = Npm.require('url');                                                                                // 220
    var parsedTargetUrl = urlModule.parse(wsUrl);                                                                      // 221
    var targetUrlPort = +parsedTargetUrl.port;                                                                         // 222
    if (!targetUrlPort) {                                                                                              // 223
      targetUrlPort = parsedTargetUrl.protocol === 'wss:' ? 443 : 80;                                                  // 224
    }                                                                                                                  // 225
                                                                                                                       // 226
    // Corporate proxy tunneling support.                                                                              // 227
    var proxyUrl = self._getProxyUrl(parsedTargetUrl.protocol);                                                        // 228
    if (proxyUrl) {                                                                                                    // 229
      var targetProtocol =                                                                                             // 230
            (parsedTargetUrl.protocol === 'wss:' ? 'https' : 'http');                                                  // 231
      var parsedProxyUrl = urlModule.parse(proxyUrl);                                                                  // 232
      var proxyProtocol =                                                                                              // 233
            (parsedProxyUrl.protocol === 'https:' ? 'Https' : 'Http');                                                 // 234
      var proxyUrlPort = +parsedProxyUrl.port;                                                                         // 235
      if (!proxyUrlPort) {                                                                                             // 236
        proxyUrlPort = parsedProxyUrl.protocol === 'https:' ? 443 : 80;                                                // 237
      }                                                                                                                // 238
      var tunnelFnName = targetProtocol + 'Over' + proxyProtocol;                                                      // 239
      var tunnelAgent = Npm.require('tunnel-agent');                                                                   // 240
      var proxyOptions = {                                                                                             // 241
        host: parsedProxyUrl.hostname,                                                                                 // 242
        port: proxyUrlPort,                                                                                            // 243
        headers: {                                                                                                     // 244
          host: parsedTargetUrl.host + ':' + targetUrlPort                                                             // 245
        }                                                                                                              // 246
      };                                                                                                               // 247
      if (parsedProxyUrl.auth) {                                                                                       // 248
        proxyOptions.proxyAuth = Npm.require('querystring').unescape(                                                  // 249
          parsedProxyUrl.auth);                                                                                        // 250
      }                                                                                                                // 251
      var tunneler = tunnelAgent[tunnelFnName]({proxy: proxyOptions});                                                 // 252
      var events = Npm.require('events');                                                                              // 253
      var fakeRequest = new events.EventEmitter();                                                                     // 254
      var Future = Npm.require('fibers/future');                                                                       // 255
      var fut = new Future;                                                                                            // 256
      fakeRequest.on('error', function (e) {                                                                           // 257
        fut.isResolved() || fut.throw(e);                                                                              // 258
      });                                                                                                              // 259
      tunneler.createSocket({                                                                                          // 260
        host: parsedTargetUrl.host,                                                                                    // 261
        port: targetUrlPort,                                                                                           // 262
        request: fakeRequest                                                                                           // 263
      }, function (socket) {                                                                                           // 264
        socket.on('close', function () {                                                                               // 265
          tunneler.removeSocket(socket);                                                                               // 266
        });                                                                                                            // 267
        process.nextTick(onConnect);                                                                                   // 268
        fut.return(socket);                                                                                            // 269
      });                                                                                                              // 270
      return fut.wait();                                                                                               // 271
    }                                                                                                                  // 272
                                                                                                                       // 273
    if (parsedTargetUrl.protocol === 'wss:') {                                                                         // 274
      return Npm.require('tls').connect(                                                                               // 275
        targetUrlPort, parsedTargetUrl.hostname, onConnect);                                                           // 276
    } else {                                                                                                           // 277
      var stream = Npm.require('net').createConnection(                                                                // 278
        targetUrlPort, parsedTargetUrl.hostname);                                                                      // 279
      stream.on('connect', onConnect);                                                                                 // 280
      return stream;                                                                                                   // 281
    }                                                                                                                  // 282
  },                                                                                                                   // 283
                                                                                                                       // 284
  _getProxyUrl: function (protocol) {                                                                                  // 285
    var self = this;                                                                                                   // 286
    // Similar to code in tools/http-helpers.js.                                                                       // 287
    var proxy = process.env.HTTP_PROXY || process.env.http_proxy || null;                                              // 288
    // if we're going to a secure url, try the https_proxy env variable first.                                         // 289
    if (protocol === 'wss:') {                                                                                         // 290
      proxy = process.env.HTTPS_PROXY || process.env.https_proxy || proxy;                                             // 291
    }                                                                                                                  // 292
    return proxy;                                                                                                      // 293
  }                                                                                                                    // 294
});                                                                                                                    // 295
